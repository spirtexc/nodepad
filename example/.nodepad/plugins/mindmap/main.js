...
// plugins/mindmap/index.ts
function parseHeadings(content) {
  const root2 = { text: "", level: 0, children: [] };
  const stack = [root2];
...
var LEVEL_COLORS = ["#89b4fa", "#cba6f7", "#f38ba8", "#fab387", "#a6e3a1", "#89dceb", "#f9e2af"];
function renderTree(container, rootData) {
  container.innerHTML = "";
  const W = container.clientWidth || 800;
  const H = container.clientHeight || 500;
  const svg = select_default2(container).append("svg").attr("width", "100%").attr("height", "100%").attr("viewBox", `0 0 ${W} ${H}`).style("display", "block");
  const g = svg.append("g");
  svg.call(
    zoom_default2().scaleExtent([0.15, 4]).on("zoom", (event) => g.attr("transform", event.transform))
  );
  const hierarchy2 = hierarchy(rootData, (d) => d.children);
  tree_default().nodeSize([34, 230])(hierarchy2);
  const nodes = hierarchy2.descendants().filter((d) => d.depth > 0);
  const ys = nodes.map((d) => d.x);
  const offsetX = 80;
  const offsetY = H / 2 - (Math.min(...ys) + Math.max(...ys)) / 2;
  g.append("g").attr("fill", "none").attr("stroke", "var(--border)").attr("stroke-width", 1.5).selectAll("path").data(hierarchy2.links().filter((l) => l.source.depth > 0)).join("path").attr(
    "d",
    linkHorizontal().x((d) => d.y + offsetX).y((d) => d.x + offsetY)
  );
  const node = g.append("g").selectAll("g").data(nodes).join("g").attr("transform", (d) => `translate(${d.y + offsetX},${d.x + offsetY})`);
  node.append("circle").attr("r", 5).attr("fill", (d) => LEVEL_COLORS[d.data.level] ?? LEVEL_COLORS[0]).attr("stroke", "var(--bg-primary)").attr("stroke-width", 2);
  node.append("text").attr("dy", "0.32em").attr("x", (d) => d.children?.length ? -10 : 10).attr("text-anchor", (d) => d.children?.length ? "end" : "start").attr("fill", "var(--text-primary)").attr("font-size", (d) => `${Math.max(10, 14 - d.depth)}px`).attr("font-family", '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif').text((d) => d.data.text.length > 42 ? d.data.text.slice(0, 40) + "\u2026" : d.data.text);
}
var STYLES = `
.mindmap-modal {
  width: 84vw;
...
`;
function buildMindmapModal(content, filename, close) {
  const root2 = parseHeadings(content);
  ...
  hint.textContent = "Scroll to zoom \xB7 Drag to pan";
  ...
  if (root2.children.length === 0) {
    ...
    empty2.className = "mindmap-empty";
    empty2.textContent = "No headings found in this file.";
  ...
  requestAnimationFrame(() => renderTree(canvas, root2));
  return wrap;
}
var styleEl = null;
var plugin = {
...
var mindmap_default = plugin;
export {
  mindmap_default as default
};
