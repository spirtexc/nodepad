...
async function buildGraph(app) {
  ...
  for (const f of files) {
    ...
    lookup.set(f.path.toLowerCase(), f.path);
    lookup.set(pathNoExt, f.path);
    lookup.set(nameNoExt, f.path);
  }
  ...
  for (const f of files) {
    let content = "";
    try {
      content = await app.readFile(f.path);
    } catch {
      continue;
    }
    for (const m2 of content.matchAll(WIKILINK_RE)) {
      const raw = m2[1].trim().toLowerCase();
...
function openGraphModal(app) {
  ...
  hint.textContent = "Click node to open \xB7 Scroll to zoom \xB7 Drag to pan";
  ...
  buildGraph(app).then(({ nodes, links }) => {
    ...
    if (nodes.length === 0) {
      ...
      empty2.className = "graph-empty";
      empty2.innerHTML = `<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" opacity="0.4"><circle cx="5" cy="5" r="2"/><circle cx="19" cy="5" r="2"/><circle cx="12" cy="19" r="2"/></svg><span>No files in vault</span>`;
      canvas.appendChild(empty2);
      return;
    }
...
function renderGraph(container, nodes, links, activeNodeId, onNodeClick) {
  ...
  const nodeRadius = (d) => Math.max(5, Math.min(18, 5 + d.degree * 2.5));
  const svg = select_default2(container).append("svg").attr("width", "100%").attr("height", "100%");
  const g = svg.append("g");
  const zoom = zoom_default2().scaleExtent([0.1, 4]).on("zoom", (event) => {
    g.attr("transform", event.transform.toString());
  });
  svg.call(zoom);
  const sim = simulation_default(nodes).force("link", link_default(links).id((d) => d.id).distance(90).strength(0.4)).force("charge", manyBody_default().strength(-220)).force("center", center_default(W / 2, H / 2)).force("collide", collide_default().radius((d) => nodeRadius(d) + 6));
  const linkSel = g.append("g").selectAll("line").data(links).join("line").attr("class", "graph-link").attr("stroke", linkColor).attr("stroke-width", 1.5);
  const nodeSel = g.append("g").selectAll("g").data(nodes).join("g").attr("class", "graph-node").call(
  ...
  nodeSel.append("circle").attr("r", nodeRadius).attr("fill", (d) => d.id === activeNodeId ? colorActive : colorDefault).attr("stroke", (d) => d.id === activeNodeId ? colorActive : "transparent").attr("stroke-width", 2);
  nodeSel.append("text").attr("dy", (d) => nodeRadius(d) + 13).attr("text-anchor", "middle").attr("fill", textColor).text((d) => d.name.length > 20 ? d.name.slice(0, 18) + "\u2026" : d.name);
  ...
  nodeSel.on("mouseenter", (_event, d) => {
    ...
    nodeSel.select("circle").attr("fill", (n) => {
    ...
    nodeSel.select("circle").attr("fill", (n) => n.id === activeNodeId ? colorActive : colorDefault).attr("opacity", 1);
...
var graph_view_default = plugin;
export {
  graph_view_default as default
};
