...
`readConfig`/`writeConfig` on the plugin `App` object (see [[core-interfaces]]) are thin wrappers over `readNodepadFile`/`writeNodepadFile` — a plugin calling `app.writeConfig('my-plugin/settings.json', ...)` ends up at `.nodepad/my-plugin/settings.json`.
