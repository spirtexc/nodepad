...
`readConfig`/`writeConfig` are the newest additions — they're how a plugin persists its own settings inside the vault (`.nodepad/<plugin-id>/...`) instead of `localStorage`/IndexedDB, so settings travel with the vault. See [[nodepad-folder]].
