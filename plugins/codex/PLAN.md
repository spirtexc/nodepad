...
> **Scoped down to what is technically reliable in a browser-only app.**
> Full semantic AI features deferred to Phase 5 (Electron + local backend).

AI watches passively and surfaces insights without being asked,
triggered by file open and file save events — not a true background process.

| Event | What fires | How | Works offline? |
|---|---|---|---|
| Open a note | Related notes found | Fuse.js search on title + headings | ✅ Yes |
| Open a note | Note not opened in 60+ days | Timestamp check in IndexedDB | ✅ Yes |
| Open a note | Missing wikilinks | Scan text for known filenames not yet linked | ✅ Yes |
| Create a note | Title overlaps existing | Fuse.js similarity score | ✅ Yes |
| Paste a URL | — | Show plain URL preview only (no AI — CORS blocked) | ✅ Yes |
| ~~Write a concept~~ | ~~You mentioned this before~~ | ❌ Needs semantic AI — deferred to Phase 5 | — |
| ~~Paste a URL~~ | ~~AI summarises it~~ | ❌ CORS blocked in browser — deferred to Phase 5 | — |

**Solves:** Forgotten notes, unlinked ideas, stale content — using only
indexes already built in the app (Fuse.js, backlinks, timestamps).
No API call on every file open — fast, free, fully offline.

> **Include in build?**
>
> **Your answer:** yes — scoped to the browser-reliable features above.
> Semantic features (concept detection, URL summarisation) added in Phase 5 Electron.
...
>
> **Your answer:** Mindmap (heading tree) + Graph View (wikilink map) — both
> already installed. Graphiti deferred to Phase 5 (requires Python backend + Neo4j,
> not browser-compatible). When available, Codex checks if each plugin is loaded
> and silently includes their output in the context block — user does not need to
> configure anything.
...
| ------ | ------------------------------------------------ | ------------------- |
| 1      | `//` inline trigger in editor                    | ✅ Build first       |
| 2      | Chat sidebar panel (vault-aware conversation)    | ✅ Build second      |
| 3      | Ambient suggestions (Fuse.js + timestamp, no AI) | ✅ Build third       |
| 4      | Structural organisation (`//auto-tag`, etc.)     | ✅ Build fourth      |
| 2+     | Plugin context sources (Mindmap + Graph View)    | ✅ Alongside Phase 2 |
| Future | Graphiti knowledge graph (Phase 5 Electron only) | ⏳ Deferred          |
